...
@export var commodity_desire_threshold := 60.0

@onready var parent := get_parent()  # For death effect + debug, could be scene root
@onready var nav : NavigationAgent3D = $NavigationAgent3D

@onready var face_material : StandardMaterial3D = $MeshInstance3D.get_surface_override_material(0)
...
func _update_velocity_for_pathed_position(delta : float):
	...
	var path_node : Vector3 = nav.get_next_path_position()
